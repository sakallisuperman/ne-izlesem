...
interface WatchedReviewModalProps {
  title: string
...
export default function WatchedReviewModal({
  title, originalTitle, mediaType, onClose, onSubmitted, zIndex = 70,
}: WatchedReviewModalProps) {
  const { user } = useAuth()
  ...
  const handleSubmit = async () => {
    ...
    if (!modResult.approved) { setMsg(modResult.reason); return }
    setSubmitting(true)
  ...
  return (
    <div
      className="fixed inset-0 flex items-end sm:items-center justify-center px-4 pb-4"
      style={{ background: '#000000bb', zIndex }}
      onClick={onClose}
    >
      <div
        className="w-full max-w-sm rounded-2xl p-5 border popup-enter"
        ...
        {done ? (
          ...
          <>
            <h3 className="text-base font-bold mb-1" style={{ color: '#f1f5f9' }}>
              Bu {mediaType === 'movie' ? 'filmi' : 'diziyi'} puanla 🎬
            </h3>
